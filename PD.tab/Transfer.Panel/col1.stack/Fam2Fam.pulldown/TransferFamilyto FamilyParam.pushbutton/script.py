# -*- coding: utf-8 -*-
__title__   = "Copy: Params from\nOther Open Family"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Select an open family and choose parameters to copy into the current family.
...
open_docs = [
    d for d in app.Documents if d.IsFamilyDocument and d.PathName != host_doc.PathName
]
...
chosen = forms.SelectFromList.show(
    choices, title="Select Source Family", button_name="Transfer"
)
...
for p in source_params:
    ...
    label = "{0} [{1}] – {2}".format(p.Definition.Name, kind, grp)
    labels.append(label)
    pmap[label] = p
