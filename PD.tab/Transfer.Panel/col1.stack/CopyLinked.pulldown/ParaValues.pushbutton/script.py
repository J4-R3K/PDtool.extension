# -*- coding: utf-8 -*-
__title__   = "Copy: Parameters from linked models"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Transfers specific parameters and values from parameters from cloud-based linked models to the main model.
...
def select_linked_model():
    ...
    model_options = {
        link.GetLinkDocument().Title: link
        for link in linked_models
    ...
    selected_model_title = forms.SelectFromList.show(
        sorted(model_options.keys()), title="Select Linked Model", button_name="Select"
    )
...
def create_project_parameter(param_name, param_group):
    ...
    if sp_file is None:
        forms.alert(
            "No shared parameter file is set. Cannot create parameter '{}'.".format(
                param_name
            )
        )
        return False
    ...
    if not param_def:
        forms.alert(
            "Parameter '{}' not found in shared parameter file. Cannot create it.".format(
                param_name
            )
        )
        return False
    ...
    if not binding_map.Insert(param_def, binding, param_group):
        ...
        if not binding_map.ReInsert(param_def, binding, param_group):
            forms.alert(
                "Failed to bind parameter '{}' to ProjectInformation.".format(
                    param_name
                )
            )
            return False
    ...
    print(
        "Parameter '{}' created and bound to ProjectInformation under group '{}'.".format(
...
def transfer_parameter_values(linked_doc, param_names):
    ...
    for name in param_names:
        ...
        if target_param is None:
            print(
                "Parameter '{}' not found in main ProjectInformation after creation. Skipping.".format(
                    name
                )
            )
            continue
    ...
    with Transaction(doc, "Transfer Project Parameters and Values") as t:
        ...
        for p_name in selected_params[:]:
            if not parameter_exists_in_main_doc(p_name):
                if create_missing:
                    param_group = param_groups.get(
                        p_name, BuiltInParameterGroup.PG_TEXT
                    )
                    success = create_project_parameter(p_name, param_group)
                    if not success:
                        print(
                            "Skipping parameter '{}' because it couldn't be created.".format(
                                p_name
                            )
                        )
                    ...
                    print(
                        "Parameter '{}' does not exist in main doc. Skipping.".format(
                            p_name
                        )
                    )
                    selected_params.remove(p_name)
