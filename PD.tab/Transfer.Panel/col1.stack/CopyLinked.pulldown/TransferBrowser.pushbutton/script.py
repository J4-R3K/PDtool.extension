# -*- coding: utf-8 -*-
__title__   = "Copy: Project Browser Template"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Transfers specific Project Browser Template from cloud-based linked models to the main model.
...
def select_browser_organizations(linked_doc):
    ...
    browser_orgs = (
        FilteredElementCollector(linked_doc).OfClass(BrowserOrganization).ToElements()
    )
    ...
    for bo in browser_orgs:
        try:
            ...
            org_name = (
                ...
                else "Unnamed (ID: {})".format(bo.Id.IntegerValue)
            )
...
def transfer_browser_organizations():
    ...
    with Transaction(doc, "Transfer Selected Project Browser Settings") as trans:
        ...
        for browser_org in browser_orgs_to_copy:
            ...
            existing_org = (
                FilteredElementCollector(doc).OfClass(BrowserOrganization).ToElements()
            )
            existing_org_names = [
                (
                    bo.LookupParameter("Type Name").AsString()
                    if bo.LookupParameter("Type Name")
                    else None
                )
            ...
            if org_name in existing_org_names:
                print(
                    "Browser Organization '{}' already exists in the main document. Skipping...".format(
                        org_name
                    )
                )
                continue
