# -*- coding: utf-8 -*-
__title__   = "Copy: View templates"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Transfers specific view templates from cloud-based linked models to the main model.
...
def transfer_view_templates():
    ...
    with Transaction(doc, "Transfer Selected View Templates") as trans:
        ...
        for view_template in view_templates_to_copy:
            ...
            existing_template = [
                et
                for et in existing_template
                if et.IsTemplate and et.Name == view_template.Name
            ]

            if existing_template:
                print(
                    "Template '{}' already exists in main document. Skipping...".format(
                        view_template.Name
                    )
                )
                continue
            ...
            if copied_ids:
                print(
                    "Successfully copied template '{}' to main document.".format(
                        view_template.Name
                    )
                )
            else:
