# -*- coding: utf-8 -*-
"""CableID: Set FedFrom from Switchboard.

Step 1 (Origin):      pick ONE source Detail Item family (any family that
                      has 'PD_DATe_ID1').
Step 2 (Destination): pick ONE OR MORE PD_DET_SC_CableID tag instances.

Writes 'PD_DATe_ID1' from the source -> 'PD_DATe_FedFrom' on each tag.
...
TARGET_PARAM_NAME = "PD_DATe_FedFrom"
...
def lookup_param_anywhere(elem, name):
    """Look on the instance first, then on its type."""
...
# ---------------------------------------------------------------------------
def main():
    output.print_md("## CableID: Set FedFrom from Switchboard")

    # ---- Step 1: origin -----------------------------------------------------
    info("Step 1 of 2 - Origin",
         "Pick the SOURCE Detail Item (e.g. PD_DET_SC_Switchboard).\n\n"
         "It must expose the parameter '{}'.".format(SOURCE_PARAM_NAME))

    src_elem = None
    while src_elem is None:
        ref = pick_one("Step 1/2: Pick SOURCE Detail Item. Esc to cancel.")
        if ref is None:
    ...
    src_value, status = read_param_string(src_elem, SOURCE_PARAM_NAME)
    if status == "missing":
        info("Aborted", "Source parameter disappeared. Aborting.")
        return

    output.print_md("**Source family:** `{}`".format(get_family_name(src_elem)))
    output.print_md("**`{}` = `{}`**".format(SOURCE_PARAM_NAME, src_value))

    # ---- Step 2: destination tags ------------------------------------------
    info("Step 2 of 2 - Destination",
         "Pick ONE OR MORE {} tags.\n\nFinish on the ribbon ('Finish').".format(TAG_FAMILY_BASE))

    tag_refs = pick_many(
        "Step 2/2: Pick {} tags (multi-select, then click Finish).".format(TAG_FAMILY_BASE)
    )
    if tag_refs is None:
        output.print_md("*Cancelled by user.*")
        return
    ...
    ok_count = 0
    fail = []
    t = Transaction(doc, "PD: Set FedFrom from Switchboard")
    t.Start()
    try:
        for tag in good:
