...
def get_base_points_from_doc(doc, model_name="Current Model"):
    ...
    for i, bp in enumerate(base_points, start=1):
        ...
        try:
            ...
            angle_to_north_param = bp.get_Parameter(
                BuiltInParameter.BASEPOINT_ANGLETON_PARAM
            )
            if angle_to_north_param and angle_to_north_param.HasValue:
                angle_to_north_radians = (
                    angle_to_north_param.AsDouble()
                )  # Angle in radians
                angle_to_north_degrees = math.degrees(
                    angle_to_north_radians
                )  # Convert to degrees
            ...
            output += (
                "{} (ID: {}) coordinates could not be retrieved.\nError: {}\n\n".format(
                    label, bp.Id.IntegerValue, e
                )
            )
...
def save_to_temp_file(content):
    ...
    forms.alert(
        "The base point coordinates have been saved to a temporary text file:\n{}".format(
            file_path
        )
    )
