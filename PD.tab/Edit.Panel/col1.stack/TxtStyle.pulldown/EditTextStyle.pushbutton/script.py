# -*- coding: utf-8 -*-
__title__   = "Text: Text Style Settings"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Modify font, size, width factor, and formatting of TextNoteTypes.
...
width_factor = round(
    style.get_Parameter(BuiltInParameter.TEXT_WIDTH_SCALE).AsDouble(), 2
)
...
with Transaction(doc, "Edit Text Style") as tx:
    ...
    style.get_Parameter(BuiltInParameter.TEXT_STYLE_BOLD).Set(
        1 if form.Result["bold"] else 0
    )
    style.get_Parameter(BuiltInParameter.TEXT_STYLE_ITALIC).Set(
        1 if form.Result["italic"] else 0
    )
    tx.Commit()
