# -*- coding: utf-8 -*-
__title__   = "Paint: Remove paint"
__doc__     = """Version = 1.0
Date    = 20.12.2025
________________________________________________________________
Description:

Removes paint from all selected Revit elements.
        ...
        for elem_id in selected_ids:
            ...
            if geometry_element:
                ...
                if processed_faces > 0:
                    out.print_html(
                        "Processed <b>{0}</b> faces on element ID <b>{1}</b>".format(
                            processed_faces, elem_id
                        )
                    )
            else:
                out.print_html(
                    "No geometry found for element ID: <b>{0}</b>".format(elem_id)
                )
        t.Commit()
